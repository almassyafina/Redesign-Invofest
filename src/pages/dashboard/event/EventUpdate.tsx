...
export default function EventUpdate() {
  ...
  const updateEvent = useEventStore((state: any) => state.updateEvent);
  ...
  useEffect(() => {
    // Tambahkan : any pada item
    const event = events.find((item: any) => item.id === Number(id));
    ...
    if (event) {
      ...
      setdateEvent(
        event.date_event instanceof Date 
  ...
  return (
    <div className="p-10 max-w-lg">
      ...
      <form onSubmit={handleUpdate} className="border-2 border-[#8B2F4A] p-6 rounded-md">
        <div className="mb-4">
        ...
        <div className="mb-4">
          ...
          <select
            value={pembicara_id}
            onChange={(e) => setPembicaraId(Number(e.target.value))}
            className="w-full border-2 border-[#8B2F4A] p-2 rounded"
            required
          >
        ...
        <button type="submit" className="border-2 bg-yellow-400 border-[#8B2F4A] px-6 py-2 rounded-3xl font-bold hover:bg-yellow-500">
          Update
        </button>
      </form>
    </div>
